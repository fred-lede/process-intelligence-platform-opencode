...
## In Progress / Next
- **CSV 範本下載（DataImport）**：`DataImport.tsx` 新增 `buildTemplateCsv()`（3 operator × 5 part × 3 reps = 45 列，欄位：lot/serial_no/datetime/machine/operator/part/thickness/temperature/pressure/**yield**/result）+ `handleDownloadTemplate()`（Blob + a.download）；匯入來源卡片「無檔」狀態新增「下載 CSV 範本」按鈕（含 Tooltip）；i18n 三語 `downloadTemplate`/`downloadTemplateDesc`。**引擎實機驗證自動偵測**：datetime→timestamp、thickness/temperature/pressure→input、**yield→output(continuous, 0.85)**、result→quality_label(binary)、machine→category、operator→metadata、part→category、serial_no→identifier、lot→category。**修正緣由**：ProcessDefine 輸出欄位候選（`ProcessDefine.tsx:78`）只收 role=output 或 quality_label **且數值**；原本的 result 是 OK/NG 二值非數值、三 input 是 input 角色 → 輸出欄位下拉為空，故新增連續型 yield 使其可選為輸出欄位並設 LSL/USL/target。涵蓋 分佈/趨勢/時間序列/GRR。tsc/build clean
