...
## In Progress / Next
- **CSV 範本下載（DataImport）**：`DataImport.tsx` 新增 `buildTemplateCsv()`（3 operator × 5 part × 3 reps = 45 列，欄位：lot/serial_no/datetime/machine/operator/part/thickness/temperature/pressure/result）+ `handleDownloadTemplate()`（Blob + a.download）；匯入來源卡片「無檔」狀態新增「下載 CSV 範本」按鈕（含 Tooltip）；i18n 三語 `downloadTemplate`/`downloadTemplateDesc`。**引擎實機驗證自動偵測**：datetime→timestamp、thickness/temperature/pressure→input、result→quality_label(binary)、machine→category、operator→metadata、part→category、serial_no→identifier、lot→category（同批所以低基數，可手動改 identifier）。涵蓋 分佈/趨勢/時間序列/GRR。tsc/build clean
