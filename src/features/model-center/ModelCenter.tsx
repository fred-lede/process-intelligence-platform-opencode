          ...
          <Space direction="vertical" style={{ width: '100%' }} size="small">
            ...
            {doeStats ? (
              <Space direction="vertical" style={{ width: '100%' }} size="small">
                {doeStats.anova ? (
                  <>
                    {doeStats.residual_diagnostics && <Alert type={doeStats.residual_diagnostics.normality_test?.is_normal && (doeStats.residual_diagnostics.durbin_watson?.statistic ?? 2) >= 1.5 && (doeStats.residual_diagnostics.durbin_watson?.statistic ?? 2) <= 2.5 && (doeStats.residual_diagnostics.outliers?.count ?? 0) === 0 ? 'success' : 'warning'} showIcon message={t('modelCenter.doeResidualDiagnostic')} description={`DW=${doeStats.residual_diagnostics.durbin_watson?.statistic.toFixed(2) ?? '—'} · p=${doeStats.residual_diagnostics.normality_test?.p_value.toFixed(3) ?? '—'} · ${t('modelCenter.doeResidualOutliers')}: ${doeStats.residual_diagnostics.outliers?.count ?? 0}`} />}
                    <div>
                    ...
                    <Card size="small" title={t('modelCenter.doeContourPlot')}>
                      ...
                      {doeContour ? <Space direction="vertical" style={{ width: '100%' }}><Plot data={[{ x: doeContour.x, y: doeContour.y, z: doeContour.z, type: 'contour', colorscale: 'Viridis', contours: { coloring: 'heatmap', showlabels: true }, colorbar: { title: 'Output' } }]} layout={{ height: 420, margin: { l: 60, r: 20, t: 20, b: 60 }, xaxis: { title: doeContour.x_factor }, yaxis: { title: doeContour.y_factor } }} /><Typography.Text type="secondary">{t('modelCenter.doeSurfaceHint')}</Typography.Text><Plot data={[{ x: doeContour.x, y: doeContour.y, z: doeContour.z, type: 'surface', colorscale: 'Viridis', colorbar: { title: 'Output' } }]} layout={{ height: 460, margin: { l: 10, r: 10, t: 20, b: 20 }, scene: { xaxis: { title: doeContour.x_factor }, yaxis: { title: doeContour.y_factor }, zaxis: { title: 'Output' } } }} /></Space> : <Alert type="info" showIcon message={t('modelCenter.doeContourHint')} style={{ marginTop: 12 }} />}
                    </Card>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: 16 }}>
                      <Card size="small" title={t('modelCenter.doeParetoChart')}>
                        <Plot data={[(() => { const rows = doeStats.coefficients.filter(c => c.name !== '1').sort((a, b) => Math.abs(b.t_stat) - Math.abs(a.t_stat)); return { x: rows.map(c => Math.abs(c.t_stat)), y: rows.map(c => c.name), type: 'bar', orientation: 'h', marker: { color: rows.map(c => Math.abs(c.t_stat) >= 2 ? '#2563eb' : '#94a3b8') }, hovertemplate: '%{y}<br>|t|=%{x:.2f}<extra></extra>' }; })()]} layout={{ height: 340, margin: { l: 150, r: 25, t: 30, b: 55 }, title: { text: 'α=0.05 · 臨界值 ≈ 2', font: { size: 12 } }, xaxis: { title: '｜t｜（標準化效應）' }, yaxis: { automargin: true }, shapes: [{ type: 'line', x0: 2, x1: 2, y0: -0.5, y1: doeStats.coefficients.filter(c => c.name !== '1').length - 0.5, line: { dash: 'dash', color: '#ef4444', width: 2 } }], showlegend: false }} />
                      </Card>
